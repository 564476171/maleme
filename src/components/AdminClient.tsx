...
const adminViewItems: { id: AdminView; label: string }[] = [
  { id: 'overview', label: '后台总览' },
  { id: 'users', label: '用户' },
  { id: 'invites', label: '邀请码' },
  { id: 'content', label: '内容' },
  { id: 'brothers', label: '兄弟人设' },
...
export default function AdminClient({ currentUser }: { currentUser: AppUser }) {
  ...
  return (
    <main className="app-shell admin-shell">
      ...
      {overview ? (
        <>
          <nav className="view-tabs admin-tabs" aria-label="后台分区">
            {adminViewItems.map((item) => (
              <button
                aria-current={activeAdminView === item.id ? 'page' : undefined}
                className="view-tab"
                key={item.id}
                type="button"
          ...
          {activeAdminView === 'overview' ? (
            <section className="view-section admin-grid">
              <section className="panel admin-card">
                <h2>注册策略</h2>
                <p className="muted">当前策略：{overview.registrationMode}</p>
              ...
              <section className="panel admin-card">
                ...
                <div className="admin-summary-grid">
                  <button type="button" onClick={() => setActiveAdminView('users')}>
                    <span>用户</span>
                    <strong>{overview.users.length}</strong>
                  </button>
                  <button type="button" onClick={() => setActiveAdminView('invites')}>
                    <span>邀请码</span>
                    <strong>{overview.invites.length}</strong>
                  </button>
                  <button type="button" onClick={() => setActiveAdminView('content')}>
                    <span>内容</span>
                    <strong>{overview.content.length}</strong>
                  </button>
                  <button type="button" onClick={() => setActiveAdminView('records')}>
                    <span>AI 记录</span>
                    <strong>{overview.reflections.length}</strong>
                  </button>
                </div>
              </section>

              <section className="panel status-panel admin-card">
                <ShieldCheck size={32} />
                <h2>资料夹正常</h2>
          ...
          {activeAdminView === 'users' ? (
            <section className="view-section single-panel-view">
              <section className="panel wide-panel admin-card">
                <h2>用户管理</h2>
          ...
          {activeAdminView === 'records' ? (
            <section className="view-section admin-records-grid">
              <section className="panel wide-panel admin-card">
                <h2>AI 记录</h2>
                <div className="admin-table">
              ...
              <section className="panel wide-panel admin-card">
                <h2>打卡记录</h2>
                <div className="admin-table">
                  {overview.checkIns.slice(0, 20).map((item) => (
                    <div className="admin-row" key={item.id}>
                      <span>{item.user?.email ?? '旧记录'}</span>
                      <strong>{item.score}/12</strong>
                      <em>{item.dateKey} / {item.level}</em>
                      <button
                        className="secondary-button"
                        type="button"
                        onClick={() =>
